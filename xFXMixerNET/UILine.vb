...
#End Region
    ...
    Private Sub CreateControls()
        Dim x As Integer
        Dim y As Integer
        Dim uilc As UILineControl = Nothing

        mText = mLine.LongName
        DeleteLineControls()

        mDeviceName = mLine.Parent.DeviceName
        If mLine.ConnectedTo <> 0 Then
            For Each l As CLine In mLine.Parent.Lines
                If l.ID = mLine.ConnectedTo Then
                    mDestinationText = l.LongName
                    Exit For
                End If
            Next l
        Else
        ...
        For Each c As CControl In mLine.Controls
            If c.CtrlItems.Count = 0 Then
                If c.ControlClass = CControl.ControlClassConstants.cccCLASS_FADER Then
                    uilc = AddFader(c, x, y)
        ...
        For Each c As CControl In mLine.Controls
            If c.CtrlItems.Count = 0 Then
                If c.ControlClass = CControl.ControlClassConstants.cccCLASS_SWITCH Then
    ...
    Private Function AddFader(ByVal c As CControl, ByVal x As Integer, ByVal y As Integer) As UILineControl
        Dim f As UILineControl = New UILineControl(c, UILineControl.UIControlTypeConstants.Fader)
        ...
        Dim cc As NMixerProNET.Controls
        Dim lp As Lines = c.Parent.Parent.LinesByComponentType(CLine.ComponentTypeConstants.ctcDST_WAVEIN)
        If lp.Count > 0 Then
            If c.Parent.ConnectedTo = lp(1).ID Then
                cc = lp(1).ControlsByType(CControl.ControlTypeConstants.ctrltcMUX)
                If cc.Count > 0 Then
                    Try
                        'Dim ci As CCtrlItem = cc(1).CtrlItems(c.Parent.Parent.Lines.Count - c.Parent.Index + 1)
                        Dim ci As CCtrlItem = cc(1).CtrlItems(lp(1).Parent.Lines.Count - lp(1).Parent.LinesByLineType(CLine.LineTypeConstants.ltcDestination).Count - c.Parent.Index + 1)

                        f = AddSwitch(ci, f.Left, f.Top + f.Height + 4)
                        f.Text = "Select"
                    Catch
                    End Try
                End If
    ...
    Private Function AddSwitch(ByVal c As CControl, ByVal x As Integer, ByVal y As Integer) As UILineControl
        Dim s As UILineControl = New UILineControl(c, UILineControl.UIControlTypeConstants.Switch)
    ...
    Private Function AddSwitch(ByVal c As CCtrlItem, ByVal x As Integer, ByVal y As Integer) As UILineControl
        Dim s As UILineControl = New UILineControl(c, UILineControl.UIControlTypeConstants.Switch)
        AddHandler s.Enter, AddressOf UILineControlGotFocus
        AddHandler s.Editing, AddressOf UILineControlEditing
        Me.Controls.Add(s)
